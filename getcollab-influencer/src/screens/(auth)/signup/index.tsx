...
export default function SignUpScreen({ navigation, route }: Props) {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      handleApiError(error, "Couldn't create your account. Try again.")
  ...
  return (
    <View style={styles.root}>
      ...
      <SafeAreaView testID="sign-up-screen" style={{ flex: 1 }} edges={['top', 'bottom']}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
          ...
          <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            <Animated.View entering={FadeInDown.duration(400)} style={styles.body}>
              <Text style={styles.eyebrow}>Join as a creator</Text>
              <Text style={styles.heading}>Create your{'\n'}creator profile</Text>
              <Text style={styles.sub}>Free forever. No card needed.</Text>

              <View style={{ gap: spacing.md, marginTop: spacing.xl }}>
                <FieldInput testID="signup-name" label="Full name" icon="person-outline" value={name} onChange={setName} error={errors.name} clearError={() => setErrors({ ...errors, name: '' })} placeholder="Your name" />
                <FieldInput testID="signup-email" label="Email" icon="mail-outline" value={email} onChange={setEmail} error={errors.email} clearError={() => setErrors({ ...errors, email: '' })} placeholder="you@example.com" keyboard="email-address" />
                <FieldInput testID="signup-password" label="Password" icon="lock-closed-outline" value={password} onChange={setPassword} error={errors.password} clearError={() => setErrors({ ...errors, password: '' })} placeholder="Min 6 characters" secure />
              </View>

              <View style={styles.terms}>
                <View style={styles.checkbox}><Ionicons name="checkmark" size={12} color={colors.black} /></View>
              ...
              <View style={styles.bottomRow}>
                ...
                <Pressable onPress={() => navigation?.navigate('SignIn')} style={({ pressed }) => pressed && { opacity: 0.85 }}>
...
function FieldInput({ label, icon, value, onChange, error, clearError, placeholder, secure, keyboard, testID }: {
  testID?: string; label: string; icon: any; value: string; onChange: (v: string) => void;
...
const styles = StyleSheet.create({
  ...
  termsText: { color: colors.textMuted, fontSize: 12, flex: 1 },
  termsLink: { color: colors.text, fontWeight: '600' },
  primaryBtn: {
    borderRadius: 999, overflow: 'hidden', marginTop: spacing.xl,
  },
  primaryInner: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 18, backgroundColor: colors.primary },
